...
var lyrics = [
  ...
  {"time": 43, "text": "Baandhun samaan apna, chal du mein"},
  {"time": 45, "text": "Kahi bhi baithun, bas tujhpe hi likhta hun"},
  {"time": 47, "text": "Dekho kitna besharam hun mein"},
  {"time": 49, "text": "Kuchal diya apne armaano ko"},
  {"time": 52, "text": "Masmaar kiya hai maikhaano ko"},
  {"time": 55, "text": "Iss baar tum aaye toh mein tumhe bhoolay se bhi ab na jaane du"},
...
var isSlowLyricsMode = false;
var lyricsInterval;
var slowLyricsSpeed = 3000; // 3 seconds per lyric
...
$(function () {
  ...
  // Add slow lyrics buttons
  $('#slowLyricsBtn').on('click', function() {
    toggleSlowLyrics();
  });

  $('#nextLyricBtn').on('click', function() {
    navigateLyrics('next');
  });

  $('#prevLyricBtn').on('click', function() {
    navigateLyrics('prev');
  });
  ...
  function updateSlider() {
    ...
    var currentVideo = $('.video-slide').eq(currentSlide).find('video')[0];
    if (currentVideo) {
      currentVideo.load();
    }
  }
  ...
  $(document).keydown(function(e) {
    switch(e.which) {
      case 37: // left arrow - previous lyric or video
        if ($('#lyricsContainer').is(':visible')) {
          navigateLyrics('prev');
        } else {
          $('.slider-btn.prev').click();
        }
        break;
      case 39: // right arrow - next lyric or video
        if ($('#lyricsContainer').is(':visible')) {
          navigateLyrics('next');
        } else {
          $('.slider-btn.next').click();
        }
        break;
      case 32: // spacebar - toggle music play/pause
        $('#playPauseBtn').click();
        e.preventDefault();
        break;
      case 83: // 's' key - toggle slow lyrics mode
        $('#slowLyricsBtn').click();
        break;
      case 76: // 'l' key - toggle lyrics display
        $('#lyricsToggleBtn').click();
        break;
      case 70: // 'f' key - favorite line
        $('#favoriteLineBtn').click();
        break;
      default: return;
    }
    e.preventDefault();
  ...
  $('#keyboardShortcutsBtn').on('click', function() {
    $('#keyboardShortcutsModal').fadeIn(500);
  });
});
...
function renderLyrics() {
  ...
  lyrics.forEach(function(line, index) {
    $('<div>').addClass('lyric-line').attr('data-index', index).text(line.text).appendTo(container);
  });
}

function updateLyrics(currentTime) {
  if (isSlowLyricsMode) return; // Don't update based on time if in slow lyrics mode
...
function toggleSlowLyrics() {
  isSlowLyricsMode = !isSlowLyricsMode;
  var audio = document.getElementById('backgroundAudio');
  
  if (isSlowLyricsMode) {
    // Enable slow lyrics mode
    $('#slowLyricsBtn').addClass('active');
    if (!audio.paused) {
      audio.pause();
      $('#playPauseIcon').removeClass('fa-pause').addClass('fa-play');
    }
    
    // Show lyrics container if not visible
    $('#lyricsContainer').fadeIn();
    $('#lyricsToggleBtn').find('i').removeClass('fa-chevron-down').addClass('fa-chevron-up');
    
    // Start slow lyrics interval
    showCurrentLyric();
    startSlowLyricsInterval();
  } else {
    // Disable slow lyrics mode
    $('#slowLyricsBtn').removeClass('active');
    clearInterval(lyricsInterval);
  }
}

function startSlowLyricsInterval() {
  clearInterval(lyricsInterval);
...
function navigateLyrics(direction) {
  if (direction === 'next') {
  ...
  // If music is playing, jump to that timestamp
  var audio = document.getElementById('backgroundAudio');
  if (!audio.paused && !isSlowLyricsMode) {
    audio.currentTime = lyrics[currentLyricIndex].time;
  }
...
function triggerLyricHeartEffect() {
  var line = lyrics[currentLyricIndex];
  $('#currentLyricDisplay').fadeOut(200, function() {
    $(this).text(line.text).fadeIn(300);
  });
  
  particlesJS('particles-js', {
    particles: {
      number: { value: 15 },
      color: { value: ['#ff4d4d', '#ff99cc'] },
